...
const fields = [
  ...
  { name: "paidMonths", label: "Meses pagados (ej: 2026-06, 2026-07)", type: "text", required: true },
  { name: "paymentMethod", label: "Medio de pago", type: "select", options: [{ value: "cash", label: "Efectivo" }, { value: "transfer", label: "Transferencia" }], required: true },
  ...
  return `
    <div class="card">
      <div class="card-header">
        ...
        <button class="btn primary" id="btn-new-payment">Registrar pago</button>
      </div>
      <div style="overflow-x:auto;">
        <table>
          <thead><tr><th>Pension</th><th>Fecha</th><th>Meses</th><th>Monto</th><th>Medio</th><th>Estado</th><th></th></tr></thead>
          <tbody>${payments.map((p) => {
            const s = stays.find((s) => s.id === p.boardingStayId);
            return `
            <tr>
              <td><strong>${escapeHtml(s?.code || p.boardingStayId)}</strong></td>
              <td>${dateLocale(p.paymentDate)}</td>
              <td>${escapeHtml(Array.isArray(p.paidMonths) ? p.paidMonths.join(", ") : p.paidMonths)}</td>
              <td>${money(p.amount)}</td>
              <td>${statusLabel(p.paymentMethod)}</td>
              <td><span class="status-pill ${statusClass(p.status)}">${statusLabel(p.status)}</span></td>
              <td>
                <div style="display:flex;gap:6px;">
                  <button class="btn ghost small" data-edit-payment="${p.id}">Editar</button>
                  ${p.status !== "cancelled" ? `<button class="btn ghost small" data-cancel-payment="${p.id}">Anular</button>` : ""}
                </div>
              </td>
            </tr>`;
          }).join("")}</tbody>
...
async function refresh() {
  const { render: r, bind: b } = await import("./payments.mjs");
  document.getElementById("view-container").innerHTML = await r();
  b();
}

function stayOpts() {
  return stays.map((s) => ({ id: s.id, label: s.code }));
}

function showCreate() {
  const html = `<form>${formFieldsHtml(fields, { paymentMethod: "transfer" }, { boardingStayId: stayOpts() })}</form>`;
...
function showEdit(id) {
  ...
  if (!p) return;
  const html = `<form>${formFieldsHtml(fields, p, { boardingStayId: stayOpts() })}</form>`;
