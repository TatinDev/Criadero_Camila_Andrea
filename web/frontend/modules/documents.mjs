...
const fields = [
  ...
  { name: "filesText", label: "Archivos (nombres, uno por linea)", type: "textarea", full: true, placeholder: "pedigree.pdf\nfoto-llegada.jpg" },
];
  ...
  return `
    <div class="card">
      ...
      <div style="overflow-x:auto;">
        <table>
          <thead><tr><th>Lote</th><th>Entidad</th><th>Archivos</th><th>Estado</th><th></th></tr></thead>
          <tbody>${batches.map((b) => `
            <tr>
              <td><strong>${escapeHtml(b.title)}</strong></td>
              <td>${escapeHtml(b.entityType)}:${escapeHtml(b.entityId)}</td>
              <td>${escapeHtml(b.files?.map((f) => f.name).join(", ") || "")}</td>
...
async function refresh() {
  const { render: r, bind: b } = await import("./documents.mjs");
  document.getElementById("view-container").innerHTML = await r();
  b();
}

function showCreate() {
  const html = `<form>${formFieldsHtml(fields, {})}<input name="fileUpload" type="file" multiple></form>`;
  modal("Subir documentos", html, async (fd) => {
    ...
    if (fileInput?.files?.length > 0) {
      ...
      for (const file of fileInput.files) {
        ...
        if (uRes && !uRes.error) names.push(file.name);
      }
...
function showEdit(id) {
  ...
  if (!b) return;
  const html = `<form>${formFieldsHtml(fields, b)}</form>`;
