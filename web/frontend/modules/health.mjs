...
function showCreateVacc() {
  modal("Registrar vacuna", `<form>${formFieldsHtml(vaccFields, {}, { horseId: horseOpts() })}</form>`, async (fd) => {
...
function showEditVacc(id) {
  ...
  if (!v) return;
  modal("Editar vacuna", `<form>${formFieldsHtml(vaccFields, v, { horseId: horseOpts() })}</form>`, async (fd) => {
    const payload = Object.fromEntries(fd.entries());
...
function showCreateFarrier() {
  modal("Registrar herraje", `<form>${formFieldsHtml(farrierFields, { serviceType: "shoeing" }, { horseId: horseOpts() })}</form>`, async (fd) => {
    const payload = Object.fromEntries(fd.entries());
...
function showEditFarrier(id) {
  const f = farrierRecords.find((item) => item.id === id);
  if (!f) return;
  modal("Editar herraje", `<form>${formFieldsHtml(farrierFields, f, { horseId: horseOpts() })}</form>`, async (fd) => {
    const payload = Object.fromEntries(fd.entries());
